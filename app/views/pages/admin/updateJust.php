...
<header class="shadow-sm bg-body position-sticky top-0 w-100">
    ...
    <div class="card shadow-sm p-3 mb-5 bg-body rounded w-50">
        <div class="card-body ">
        <form action="<?php echo URLROOT ?>/SeniorController/updateJustById/<?php echo $data[1]["just"]->id ?>" method="POST">
        <div class="mb-3 w-100">
                    <label for="exampleInputEmail1" class="form-label">justification</label>
                    <input type="text" class="form-control" id="exampleInputEmail1" value="<?php echo $data[1]["just"]->justification ?>"  name="justification">          
                </div>
        



            <button class="btn btn-danger d-block btn-user w-100" type="submit">Modifier</button>
        </form>
        </div>
    </div>
